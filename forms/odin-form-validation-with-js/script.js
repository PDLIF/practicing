...
const zipCodeConstraints = {
    ...
    sw: [
        /^\d{4}$/,
        "Swiss ZIP code must be exactly 4 digits (e.g., 8000).",
    ],
    de: [
        /^\d{5}$/,
        "German ZIP code must be exactly 5 digits (e.g., 10115)",
    ],
    nl: [
        /^\d{4} ?[A-Z]{2}$/,
...
const checkPassword = () => {
    ...
    if (passwordRegExp.test(passwordInput.value) && passwordInput.value !== '') {
        ...
        passwordInput.setCustomValidity('Password should be at least 8 characters, 1 letter, 1 number, 1 special character');
